module Notesack.EditStr ( 
  EditStr, snapCursor, isValidCursor, insert, delete, backspace, toLines, toText, fromText
) where 
...
backspace e cursor | not (isValidCursor e cursor) = error "invalid cursor"
backspace e (0,0) = ((0,0),e)
--backspace (E nx items) (x,y) | y == length items = ((nx-1,y-1), E nx newItems)
--  where (lhs, rhs) = Seq.splitAt (y-1) items
--        (rowNum, line) = Seq.index rhs 0 
--        newLine = Seq.deleteAt (nx-1) line
--        newItems = lhs |> (rowNum, newLine)
backspace (E nx items) (0,y) = (newCursor, E nx newItems)
  where (prevRowNum, prevLine) = Seq.index items' (y-1)
        -- if we're on a new row, but we don't know it, add the row
        items' = if y == Seq.length items 
                    then items |> (prevRowNum + 1, Seq.empty)
                    else items
        (rowNum, line) = Seq.index items' y
        (newCursor, newItems) =
          if (prevRowNum == rowNum)
             then let (lhs,rhs) = Seq.splitAt (y-1) items'
                      toAdd = Seq.deleteAt (nx-1) prevLine >< line
                   in ((nx-1,y-1), lhs >< recurse nx rowNum toAdd (Seq.drop 2 rhs))
             else let (lhs,rhs) = Seq.splitAt (y-1) items'
                      toAdd = prevLine >< line
                   in ((Seq.length prevLine, y-1), 
                       lhs >< recurse nx prevRowNum toAdd (decrementRow (Seq.drop 2 rhs)))

backspace (E nx items) (x,y) = ((x-1,y), E nx newItems) 
  where (lhs,rhs) = Seq.splitAt y items
        (rowNum, line) = Seq.index rhs 0
        toAdd = Seq.deleteAt (x-1) line
        newItems = lhs >< recurse nx rowNum toAdd (Seq.drop 1 rhs)
